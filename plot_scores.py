...
#             Dataset with information about Ground truth ('value_y') and predictions ('value_x') 
    ...
    #                 Find std from quantiles
                ...
                if(len(tg_vals)==0):
                    print("No target found for week ahead ",i, " region ", region, "model ",model)

#             Calculate ls and crps
                if(0 in std_):
                    cr_target = np.array([EPS if x ==0 else x for x in tg_vals], dtype=object).reshape((len(tg_vals), 1))
